...
namespace inventory_system_api.unitTests
{
    public class SalesInvoiceControllerTests
    {
        ...
        public async Task Post_ReturnsOk_OnSuccess()
        {
            ...
            _mockRepo.Setup(r => r.AddEdit(It.IsAny<SalesInvoiceMaster>(), 1, cancellationToken)).ReturnsAsync(1);
            ...
            Assert.Equal(200, resp.StatusCode);
            Assert.Equal("Success", resp.Message);
            _mockRepo.Verify(r => r.AddEdit(It.IsAny<SalesInvoiceMaster>(), 1, cancellationToken), Times.Once);
        }
        ...
        {
            _mockRepo.Setup(r => r.Delete(3, 1, cancellationToken)).ReturnsAsync(1);
            ...
            Assert.Equal(200, resp.StatusCode);
            Assert.Equal("Success", resp.Message);
            _mockRepo.Verify(r => r.Delete(3, 1, cancellationToken), Times.Once);
        }
    }
}
